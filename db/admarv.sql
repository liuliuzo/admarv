DROP TABLE IF EXISTS sys_user;
CREATE TABLE `sys_user` (
  `id` INT AUTO_INCREMENT COMMENT '主键id',
  `user_name` varchar(100) DEFAULT NULL COMMENT '登录账号',
  `real_name` varchar(100) DEFAULT NULL COMMENT '真实姓名',
  `password` varchar(255) DEFAULT NULL COMMENT '密码',
  `salt` varchar(45) DEFAULT NULL COMMENT 'md5密码盐',
  `avatar` varchar(255) DEFAULT NULL COMMENT '头像',
  `birthday` datetime DEFAULT NULL COMMENT '生日',
  `sex` tinyint(1) DEFAULT NULL COMMENT '性别(0-默认未知,1-男,2-女)',
  `email` varchar(45) DEFAULT NULL COMMENT '电子邮件',
  `phone` varchar(45) DEFAULT NULL COMMENT '电话',
  `org_code` varchar(64) DEFAULT NULL COMMENT '登录会话的机构编码',
  `status` tinyint(1) DEFAULT NULL COMMENT '性别(1-正常,2-冻结)',
  `third_id` varchar(100) DEFAULT NULL COMMENT '第三方登录的唯一标识',
  `third_type` varchar(100) DEFAULT NULL COMMENT '第三方类型',
  `telephone` varchar(45) DEFAULT NULL COMMENT '座机号',
  `fb_token` varchar(1024) DEFAULT NULL COMMENT 'facebook access token',
  `del_flag` tinyint(1) DEFAULT NULL COMMENT '删除状态(0-正常,1-已删除)',
  `create_by` varchar(32) DEFAULT NULL COMMENT '创建人',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_by` varchar(32) DEFAULT NULL COMMENT '更新人',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE KEY `uniq_sys_user_user_name` (`user_name`) USING BTREE,
  UNIQUE KEY `uniq_sys_user_phone` (`phone`) USING BTREE,
  UNIQUE KEY `uniq_sys_user_email` (`email`) USING BTREE,
  KEY `idx_user_name` (`user_name`) USING BTREE,
  KEY `idx_status` (`status`) USING BTREE,
  KEY `idx_del_flag` (`del_flag`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=1 COMMENT='用户表';

DROP TABLE IF EXISTS `lead_gen`;
CREATE TABLE `lead_gen` (
  `id` INT AUTO_INCREMENT PRIMARY KEY,
  `lead_id` VARCHAR(255),
  `name` VARCHAR(255),
  `email` VARCHAR(255),
  `cntct` VARCHAR(255),
  `crte_tm` VARCHAR(255),
  `lead_stat` VARCHAR(255),
  `regn` VARCHAR(255),
  `flwp_stat` VARCHAR(255),
  `rsrc` VARCHAR(255),
  `form_id` VARCHAR(255),
  `owner` VARCHAR(255) COMMENT '负责人',
  `user_name` VARCHAR(255) COMMENT '账户所有人',
  `del_flag` tinyint(1) DEFAULT NULL COMMENT '删除状态(0-正常,1-已删除)',
  `create_by` varchar(32) DEFAULT 'system' COMMENT '创建人',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_by` varchar(32) DEFAULT 'system' COMMENT '更新人',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间'
) ENGINE=InnoDB AUTO_INCREMENT=1 COMMENT='询盘单表';

DROP TABLE IF EXISTS `campaigns_insights`;
CREATE TABLE campaigns_insights (
    'id' INT PRIMARY KEY AUTO_INCREMENT,
    'account_currency' VARCHAR(255),
    'account_id' VARCHAR(255),
    'account_name' VARCHAR(255),
    'action_values' VARCHAR(255),
    'actions' INT,
    'activity_recency' INT,
    'ad_click_actions' INT,
    'ad_format_asset' VARCHAR(255),
    'ad_id' VARCHAR(255),
    'ad_impression_actions' INT,
    'ad_name' VARCHAR(255),
    'adset_id' VARCHAR(255),
    'adset_name' VARCHAR(255),
    'age_targeting' VARCHAR(255),
    'attribution_setting' VARCHAR(255),
    'auction_bid' DOUBLE,
    'auction_competitiveness' DOUBLE,
    'auction_max_competitor_bid' DOUBLE,
    'body_asset' VARCHAR(255),
    'buying_type' VARCHAR(255),
    'campaign_id' VARCHAR(255),
    'campaign_name' VARCHAR(255),
    'canvas_avg_view_percent' DOUBLE,
    'canvas_avg_view_time' DOUBLE,
    'catalog_segment_actions' INT,
    'catalog_segment_value' DOUBLE,
    'catalog_segment_value_mobile_purchase_roas' DOUBLE,
    'catalog_segment_value_omni_purchase_roas' DOUBLE,
    'catalog_segment_value_website_purchase_roas' DOUBLE,
    'clicks' INT,
    'coarse_conversion_value' DOUBLE,
    'comparison_node' VARCHAR(255),
    'conversion_values' DOUBLE,
    'conversions' INT,
    'converted_product_quantity' INT,
    'converted_product_value' DOUBLE,
    'cost_per_15_sec_video_view' DOUBLE,
    'cost_per_2_sec_continuous_video_view' DOUBLE,
    'cost_per_action_type' DOUBLE,
    'cost_per_ad_click' DOUBLE,
    'cost_per_conversion' DOUBLE,
    'cost_per_dda_countby_convs' DOUBLE,
    'cost_per_inline_link_click' DOUBLE,
    'cost_per_inline_post_engagement' DOUBLE,
    'cost_per_one_thousand_ad_impression' DOUBLE,
    'cost_per_outbound_click' DOUBLE,
    'cost_per_thruplay' DOUBLE,
    'cost_per_unique_action_type' DOUBLE,
    'cost_per_unique_click' DOUBLE,
    'cost_per_unique_conversion' DOUBLE,
    'cost_per_unique_inline_link_click' DOUBLE,
    'cost_per_unique_outbound_click' DOUBLE,
    'country' VARCHAR(255),
    'cpc' DOUBLE,
    'cpm' DOUBLE,
    'cpp' DOUBLE,
    'created_time' BIGINT,
    'ctr' DOUBLE
);
